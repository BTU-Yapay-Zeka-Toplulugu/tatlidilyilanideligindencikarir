...
// Kayıt Ol Sayfası — Tam Ekran Minimalist Tasarım
// Named Export — export default YASAKTIR (ADR-001)
// Özellikler: Minimalist responsive siyah/beyaz kart, focus:ring-gray-500 inputları, dil desteği, anasayfa linki, sosyal giriş, form doğrulama, dikey sığma optimizasyonu

import { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Bot, ArrowLeft, User, Mail, Lock, Eye, EyeOff, Sun, Moon } from 'lucide-react';
...
const TRANSLATIONS = {
  ...
  en: {
    title: "Sign Up",
    subtitle: "Create a new account by filling in the details below.",
    nameLabel: "Full Name",
...
export const Register = () => {
  ...
  // Validation State
  const [nameHata, setNameHata] = useState('');
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      // Simülasyon: Kayıt başarılı ise Giriş sayfasına yönlendir
  ...
  return (
    <div className="relative min-h-screen flex flex-col justify-center items-center px-4 bg-white text-black dark:bg-black dark:text-white transition-colors duration-300">
      ...
      {/* Kayıt Kartı (Mobil için p-4, masaüstü için sm:p-6 yapıldı) */}
      <div className="w-full max-w-md mx-auto rounded-3xl border border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/30 backdrop-blur-sm p-4 sm:p-6 shadow-sm">
        
        {/* Logo & Başlık (Mobil için mb-4, logo altı mb-2 yapıldı) */}
        <div className="flex flex-col items-center text-center mb-4 sm:mb-6">
          <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-black dark:bg-white flex items-center justify-center text-white dark:text-black shadow-md mb-2 sm:mb-3">
            <Bot size={18} className="sm:h-5 sm:w-5" />
          </div>
          <h1 className="text-lg sm:text-xl font-bold tracking-tight">{t('title')}</h1>
          <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400 max-w-[280px]">
            {t('subtitle')}
          </p>
        </div>

        {/* Form (Dikey sığma için space-y-3 sm:space-y-4 yapıldı) */}
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4" noValidate>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  "w-full pl-9 pr-4 py-2 sm:py-2.5 rounded-xl border bg-white dark:bg-black text-sm focus:outline-none transition-all placeholder:text-neutral-400",
                  nameHata
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  "w-full pl-9 pr-10 py-2 sm:py-2.5 rounded-xl border bg-white dark:bg-black text-sm focus:outline-none transition-all placeholder:text-neutral-400",
                  sifreHata
                    ? "border-red-500 ring-1 ring-red-500"
                    : "border-neutral-200 dark:border-neutral-800 focus:ring-1 focus:ring-gray-500"
                )}
          ...
          <button
            type="submit"
            className="w-full py-2.5 sm:py-3 mt-1 rounded-xl text-sm font-semibold shadow-md bg-black text-white hover:bg-neutral-900 dark:bg-white dark:text-black dark:hover:bg-neutral-100 transition-all active:scale-[0.99] focus:outline-none"
          >
            {t('submitBtn')}
          </button>
        </form>

        {/* Ayraç (Dikey sığma için my-3 yapıldı) */}
        ...
        <button
          ...
          className="w-full py-2 sm:py-2.5 px-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-transparent hover:bg-neutral-100 dark:hover:bg-neutral-900 text-sm font-semibold transition-all flex items-center justify-center gap-2.5 focus:outline-none"
        >
        ...
        <div className="mt-3 sm:mt-4 text-center text-xs text-neutral-500 dark:text-neutral-400">
      ...
      {/* Ana Sayfaya Dön (Mobilde dipte sıkışmaması için mb-6 eklenmiştir) */}
      <Link
        to={ROUTES.LANDING}
        className="mt-4 sm:mt-6 mb-6 sm:mb-0 flex items-center gap-1.5 text-xs font-semibold text-neutral-500 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors focus:outline-none"
      >
        <ArrowLeft size={14} />
